  ...
  return (
    <SettingsSection title={t("settings.password.title", locale)}>
      <form action={action} className="space-y-4" key={state?.ok ? "ok" : "form"}>
        ...
        <button disabled={pending} className="text-white rounded-[11px] py-2.5 px-5 text-sm font-medium disabled:opacity-60" style={{ background: "linear-gradient(180deg, #4C1D95 0%, #9F1239 55%, #BE123C 100%)" }}>
  ...
  return (
    <div>
      ...
      <div className="relative">
        <input name={name} type={show ? "text" : "password"} required minLength={minLength} className={`w-full px-3.5 py-3 rounded-[10px] text-sm outline-none focus:border-violet-700 ${hideToggle ? "" : "pr-11"}`} style={{ background: "#fdfbf4", border: "1px solid rgba(31,36,32,0.15)", color: "#1F2420" }} />
        {!hideToggle && (
