    ...
    <SettingsSection title={t("settings.profile.title", locale)}>
      <form action={action} className="space-y-4">
        <div>
          ...
          <input value={email} readOnly className="w-full px-3.5 py-3 rounded-[10px] text-sm cursor-not-allowed" style={{ background: "#fdfbf4", border: "1px solid rgba(31,36,32,0.10)", color: "#7A7268" }} />
        ...
        <div>
          ...
          <input name="name" value={n} onChange={e => setN(e.target.value)} placeholder={t("settings.profile.name_placeholder", locale)} className="w-full px-3.5 py-3 rounded-[10px] text-sm outline-none transition-colors focus:border-violet-700" style={{ background: "#fdfbf4", border: "1px solid rgba(31,36,32,0.15)", color: "#1F2420" }} />
        </div>
        ...
        <button disabled={pending || n === name} className="text-white rounded-[11px] py-2.5 px-5 text-sm font-medium transition-all disabled:opacity-40 disabled:cursor-not-allowed" style={{ background: "linear-gradient(180deg, #4C1D95 0%, #9F1239 55%, #BE123C 100%)" }}>
