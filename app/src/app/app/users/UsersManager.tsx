...
export default function UsersManager({ currentUserId }: { currentUserId: string }) {
  ...
  return (
    <div className="max-w-3xl mx-auto w-full p-6">
      ...
      <p className="text-sm text-stone-500 mb-6">Lad Kollegen zu Syn ein und verwalte Accounts.</p>

      <div className="rounded-2xl border border-stone-200 glass p-5 mb-6">
        <label className="text-xs uppercase tracking-wide text-stone-500">Email-Adresse einladen</label>
        <div className="flex gap-2 mt-2">
          <input value={email} onChange={e => setEmail(e.target.value)}
            placeholder="kollege@firma.com" type="email"
            onKeyDown={e => { if (e.key === "Enter") create(); }}
            className="flex-1 px-4 py-2.5 rounded-xl bg-white/80 border border-stone-300 focus:outline-none focus:border-rose-700/50" />
      ...
      <h2 className="text-sm font-medium text-stone-600 mb-3 uppercase tracking-wide">Accounts</h2>
      <ul className="space-y-2 mb-8">
        {users.map(u => {
          const isSelf = u.id === currentUserId;
          return (
            <li key={u.id} className="rounded-xl border border-stone-200 bg-white/50 p-3 flex items-center justify-between">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="font-medium truncate">{u.name || u.email}</span>
                  {u.isAdmin && <span className="text-[10px] uppercase px-2 py-0.5 rounded-full bg-rose-800/80 text-rose-800 border border-rose-700/40">Admin</span>}
      ...
      {pendingInvites.length > 0 && (
        <>
          <h2 className="text-sm font-medium text-stone-600 mb-3 uppercase tracking-wide">Offene Einladungen</h2>
          <ul className="space-y-2">
            {pendingInvites.map(i => {
              const expired = new Date(i.expiresAt) < new Date();
              return (
                <li key={i.id} className="rounded-xl border border-stone-200 bg-amber-50/50 p-3 flex items-center justify-between">
                  <div className="min-w-0 flex-1">
                    <div className="font-medium truncate">{i.email}</div>
                    <div className="text-xs text-stone-500 mt-0.5">{expired ? "Abgelaufen" : "Gueltig"}</div>
                  </div>
