...
function rigidityLabel(r: number, locale: Locale = "de"): string {
  if (r <= 3) return t("stance.offen", locale);
  if (r <= 6) return t("stance.ausgewogen", locale);
  return t("stance.standhaft", locale);
}
...
export default function PersonaSidebar({ sessionId, refreshToken, onSelect, locale = "de" }: Props) {
  ...
  return (
    <aside className="w-72 glass-card-dark p-3 space-y-2 overflow-y-auto border-r border-stone-300/60">
      <div className="flex items-center justify-between mb-2 px-1">
        <div className="text-xs uppercase tracking-wide text-stone-700 font-bold">{t("sidebar.personas", locale)}</div>
        <button onClick={toggleCollapsed}
          title={t("sidebar.collapse", locale)}
          ...
          <div key={n}
              ...
              <button onClick={() => onSelect(n)}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-stone-600 mt-0.5 line-clamp-1">
                    {p ? (p.type && p.type.toLowerCase() !== "human" ? p.type : t("sidebar.persona", locale)) : t("sidebar.notAssigned", locale)}
                  </div>
                </div>
              </button>
              ...
              return (
                <div className="border-t border-stone-200 px-3 py-2 bg-white/40 pl-4"
                  ...
                  <div className="flex items-center justify-between text-[10px] uppercase tracking-wide font-bold mb-1">
                    <span className="text-stone-700">{t("sidebar.haltung", locale)}</span>
                    <span className="normal-case tracking-normal font-bold" style={{ color: stops.bottom }}>{rigidityLabel(current, locale)}</span>
                  </div>
                  ...
                  <div className="flex justify-between text-[10px] text-stone-600 font-semibold mt-0.5">
                    <span>{t("stance.offen", locale)}</span>
                    <span>{t("stance.standhaft", locale)}</span>
                  </div>
                </div>
              );
            })()}
            {isExp && (
              <div className="border-t border-stone-200 p-3 space-y-2 text-xs bg-stone-50 pl-4">
                {p.core_perspective && (
                  <div><div className="text-stone-500 font-medium">{t("sidebar.perspektive", locale)}</div>
                    <div className="text-stone-800 whitespace-pre-wrap">{p.core_perspective}</div></div>
                )}
                {p.profile && (
                  <div><div className="text-stone-500 font-medium">{t("sidebar.profil", locale)}</div>
                    <div className="text-stone-700 whitespace-pre-wrap">{p.profile}</div></div>
                )}
                {p.position_summary && (
                  <div><div className="text-stone-500 font-medium">{t("sidebar.position", locale)}</div>
          ...
          {syntheses.sort((a,b) => a.round_number - b.round_number).map(s => (
            <div key={s.round_number} className="relative rounded-2xl bg-[#F3EFE2] border border-stone-300/60 mb-2 overflow-hidden shadow-sm hover:shadow-md transition-all">
              ...
              <button onClick={() => setSynthOpen(synthOpen === s.round_number ? null : s.round_number)}
                ...
                <div className="font-semibold" style={{ color: SYNTH_ACCENT.bottom }}>{t("sidebar.runde", locale)} {s.round_number}</div>
              </button>
